...
function Tech({ tech }) {
  ...
  //       }, [handleScroll]);
    function FnMakeGraphTemplate (score, skill, idx) {
        console.log("SCORE",score)
        return(
            <S.Li key={idx}>
            <S.Graph
              {...useScrollGraph(score)}
            >
              <S.Span>
                <img
                  src={imgArr[skill]}
              ...
              <S.Span size="0.8" className="skillName"> {skill}</S.Span>
            </S.Graph>
    ...
    <Box.Article>
      <Box.Div>
        <Box.DivItem>
          ...
          <div>
            <S.Ul>
              {tech.map((item, idx) =>  FnMakeGraphTemplate(item.score, item.Lang, idx))}
            </S.Ul>
          </div>
        </Box.DivItem>
      </Box.Div>
